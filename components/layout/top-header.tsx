...
export function TopHeader({ onOpenCommandPalette, onOpenAuth }: { onOpenCommandPalette: () => void, onOpenAuth?: () => void }) {
  ...
  return (
    <header className="absolute top-0 right-0 h-[72px] flex items-center justify-between px-4 md:px-8 z-20 w-full bg-background/50 backdrop-blur-xl pointer-events-none border-b border-borders">
      ...
      <div className="pointer-events-auto flex-1 flex items-center justify-start">
        <Link href="/home" className="md:hidden flex items-center gap-2">
          ...
          <span className="font-bold tracking-[0.2em] text-[13px] text-foreground uppercase mt-0.5">NEXORA</span>
        </Link>
      </div>
      ...
      <div className="pointer-events-auto flex-shrink-0 flex items-center justify-center">
        <button 
          ...
          className="flex items-center justify-between w-[180px] sm:w-[280px] md:w-[340px] text-textMuted bg-hoverBg hover:bg-hoverBg/80 px-3 md:px-4 py-2 md:py-2.5 rounded-xl border border-borders transition group"
        >
          <div className="flex items-center gap-2 md:gap-3 overflow-hidden">
            <Search size={16} className="text-textMuted group-hover:text-foreground transition shrink-0" />
            <span className="text-[12px] md:text-[13px] font-medium group-hover:text-foreground transition truncate">Search...</span>
          </div>
          <kbd className="hidden sm:inline-flex items-center gap-1 bg-bgDarker border border-borders rounded px-1.5 py-0.5 text-[10px] font-mono text-textMuted">
            <Command size={10} /> K
          </kbd>
        </button>
      </div>
      ...
      <div className="pointer-events-auto flex-1 flex items-center gap-3 md:gap-5 justify-end">
        <button className="text-textMuted hover:text-foreground transition hidden sm:block">
          <Gift size={20} />
        </button>
        ...
        <div className="w-px h-6 bg-borders mx-1"></div>

        {/* User Profile Dropdown */}
        <div className="relative" ref={dropdownRef}>
          <button 
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className={`transition w-9 h-9 flex items-center justify-center rounded-full border border-borders overflow-hidden ${
              isDropdownOpen 
          ...
          >
            {user ? (
              <img 
                ...
                alt="Avatar" 
                className="w-full h-full object-cover bg-hoverBg"
              />
            ) : (
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 glass-panel border border-borders rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] overflow-hidden animate-in slide-in-from-top-2 fade-in duration-200">
              {user && (
                <div className="px-4 py-3 border-b border-borders bg-hoverBg">
              ...
              <div className="py-1">
                <Link 
                  ...
                  className="flex items-center gap-3 px-4 py-2 text-sm text-foreground hover:bg-hoverBg transition-colors"
                >
                  <Settings size={16} />
                  Settings
                </Link>
