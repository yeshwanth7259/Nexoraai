...
export function InlinePrompt() {
  ...
  return (
    <div className="relative z-10 w-full max-w-2xl bg-bgDarker/80 backdrop-blur-xl border border-borders rounded-2xl p-2 shadow-2xl flex flex-col gap-2 mx-auto">
        ...
        {attachedFile && (
          ...
          >
            <div className="relative inline-flex items-center gap-3 bg-hoverBg border border-borders rounded-xl p-2 pr-4">
              <button 
                onClick={removeFile}
                className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-slate-700 hover:bg-red-500 text-foreground flex items-center justify-center transition z-10"
              >
                <X size={12} />
              </button>
      ...
      <div className="flex items-center justify-between px-2 sm:px-3 pt-2">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <button 
            ...
            className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center transition shadow-[0_0_15px_rgba(109,91,255,0.5)] ${input.trim() || attachedFile ? 'bg-primary hover:bg-accent text-foreground' : 'bg-primary/50 text-foreground/50 cursor-not-allowed'}`}
          >
